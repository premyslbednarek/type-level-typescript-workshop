 ...
 */
namespace one {
  type Head<tuple extends any[]> = tuple extends [infer T, ...any[]] ? T : never;
 ...
 */
namespace two {
  type DropFirst<tuple extends any[]> = tuple extends [any, ...infer T] ? T : [];
...
namespace bonus {
   ...
   */
  namespace six {
    type DropLast<tuple extends any[]> = tuple extends [...infer everythingButLast, any] ? everythingButLast : [];
   ...
   */
  namespace eight {
    type IsTuple<tuple> = tuple extends [] | [any, ...any] ? true : false; // had to look this up in the solution
